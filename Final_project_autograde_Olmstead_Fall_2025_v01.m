...
if ~isnan(radius)
    ...
    elseif radius >= 410 - distTol
        logText = logf(logText, 'Mission radius meets objective (410 nm) [+1 bonus]: %.1f nm\n', radius);
    end
end
...
if Main(3,22) < 1 - tol
    logText = logf(logText, 'MaxMach: n = %.3f g, expected %.3f g\n', Main(3,22), 1);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.MaxMach = true;
end
...
if abs(Main(3,24)) > tol
    logText = logf(logText, 'MaxMach: Ps = %.0f ft/s, expected %.0f ft/s\n', Main(3,24), 0);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.MaxMach = true;
end
if abs(Main(3,19) - betaDefault) > 1e-3
    logText = logf(logText, 'MaxMach: W/WTO expected %.3f for 50%% fuel load; found %.3f. Please update to 50%% fuel load weight fraction.\n', betaDefault, Main(3,19));
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.MaxMach = true;
end
...
if Main(4,22) < 1 - tol
    logText = logf(logText, 'CruiseMach: n = %.3f g, expected %.3f g\n', Main(4,22), 1);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.CruiseMach = true;
end
...
if abs(Main(4,24)) > tol
    logText = logf(logText, 'CruiseMach: Ps = %.0f ft/s, expected %.0f ft/s\n', Main(4,24), 0);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.CruiseMach = true;
end
if abs(Main(4,19) - betaDefault) > 1e-3
    logText = logf(logText, 'CruiseMach: W/WTO expected %.3f for 50%% fuel load; found %.3f. Please update to 50%% fuel load weight fraction.\n', betaDefault, Main(4,19));
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.CruiseMach = true;
end
...
if Main(6,22) < 3 - tol
    logText = logf(logText, 'Cmbt Turn1: g-load = %.3f g, must be >= %.1f g\n', Main(6,22), 3.0);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.CmbtTurn1 = true;
...
if abs(Main(6,24)) > tol
    logText = logf(logText, 'Cmbt Turn1: Ps = %.0f ft/s, expected %.0f ft/s\n', Main(6,24), 0);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.CmbtTurn1 = true;
end
if abs(Main(6,19) - betaDefault) > 1e-3
    logText = logf(logText, 'Cmbt Turn1: W/WTO expected %.3f for 50%% fuel load; found %.3f. Please update to 50%% fuel load weight fraction.\n', betaDefault, Main(6,19));
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.CmbtTurn1 = true;
end
...
if Main(7,22) < 4 - tol
    logText = logf(logText, 'Cmbt Turn2: g-load = %.3f g, must be >= %.1f g\n', Main(7,22), 4.0);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.CmbtTurn2 = true;
...
if abs(Main(7,24)) > tol
    logText = logf(logText, 'Cmbt Turn2: Ps = %.0f ft/s, expected %.0f ft/s\n', Main(7,24), 0);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.CmbtTurn2 = true;
end
if abs(Main(7,19) - betaDefault) > 1e-3
    logText = logf(logText, 'Cmbt Turn2: W/WTO expected %.3f for 50%% fuel load; found %.3f. Please update to 50%% fuel load weight fraction.\n', betaDefault, Main(7,19));
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.CmbtTurn2 = true;
end
...
if Main(8,22) < 1 - tol
    logText = logf(logText, 'Ps1: n = %.3f g, expected %.3f g\n', Main(8,22), 1);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.Ps1 = true;
end
...
if Main(8,24) < 400 - distTol
    logText = logf(logText, 'Ps1: Ps = %.0f ft/s, must be >= %.0f ft/s\n', Main(8,24), 400);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.Ps1 = true;
...
if abs(Main(8,19) - betaDefault) > 1e-3
    logText = logf(logText, 'Ps1: W/WTO expected %.3f for 50%% fuel load; found %.3f. Please update to 50%% fuel load weight fraction.\n', betaDefault, Main(8,19));
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.Ps1 = true;
end
...
if Main(9,22) < 1 - tol
    logText = logf(logText, 'Ps2: n = %.3f g, expected %.3f g\n', Main(9,22), 1);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.Ps2 = true;
end
...
if Main(9,24) < 400 - distTol
    logText = logf(logText, 'Ps2: Ps = %.0f ft/s, must be >= %.0f ft/s\n', Main(9,24), 400);
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.Ps2 = true;
...
if abs(Main(9,19) - betaDefault) > 1e-3
    logText = logf(logText, 'Ps2: W/WTO expected %.3f for 50%% fuel load; found %.3f. Please update to 50%% fuel load weight fraction.\n', betaDefault, Main(9,19));
    constraintErrors = constraintErrors + 1;
    rowErrorsMap.Ps2 = true;
end
...
if abs(Main(12,22) - 0.03) > 5e-4
    logText = logf(logText, 'Takeoff: n = %.3f g, expected %.3f g\n', Main(12,22), 0.03);
    constraintErrors = constraintErrors + 1;
end
...
elseif ~isnan(takeoff_dist) && takeoff_dist <= 2500 + distTol
    logText = logf(logText, 'Takeoff distance meets objective (<= 2500 ft) [+1 bonus]: %.0f ft\n', takeoff_dist);
end
...
if abs(Main(13,22) - 0.5) > tol
    logText = logf(logText, 'Landing: n = %.3f g, expected %.3f g\n', Main(13,22), 0.5);
    constraintErrors = constraintErrors + 1;
end
...
elseif ~isnan(landing_dist) && landing_dist <= 3500 + distTol
    logText = logf(logText, 'Landing distance meets objective (<= 3500 ft) [+1 bonus]: %.0f ft\n', landing_dist);
end
...
aim120Int = round(aim120);
aim9Int = round(aim9);
if abs(aim120 - aim120Int) > payloadTol || abs(aim9 - aim9Int) > payloadTol
    pt = pt - 4;
    logText = logf(logText, '-4 pts Payload counts must be whole missiles: AIM-120 = %.2f, AIM-9 = %.2f\n', aim120, aim9);
elseif aim120Int < 8 - tol
    count = aim120Int;
    pt = pt - 4;
    logText = logf(logText, '-4 pts Payload must include at least 8 AIM-120Ds (found %.0f)\n', count);
elseif aim9Int >= 2 - tol
...
if stabilityDeduction > 0
    ...
    logText = logf(logText, '-%d pts Unstable! Adjust your aircraft to achieve flyable stability parameters by fixing red cells in Main M10-Q10.\n', stabilityDeduction);
end

% Fuel (2 pts) and volume (2 pts)
if isnan(fuel_available) || isnan(fuel_required) || fuel_available + tol < fuel_required
    pt = pt - 2;
    logText = logf(logText, '-2 pts Fuel available (%.1f lb) is less than required (%.1f lb)\n', fuel_available, fuel_required);
end

if isnan(volume_remaining) || volume_remaining <= 0
    pt = pt - 2;
    logText = logf(logText, '-2 pts Volume remaining must be positive (Q23 = %.2f ft^3)\n', volume_remaining);
end
...
noseLoadPct = Gear(19, 10);
if isnan(noseLoadPct) || noseLoadPct < 10 - gearPercentTol || noseLoadPct > 20 + gearPercentTol
...
if isnan(tipbackActual) || isnan(tipbackLimit) || tipbackActual >= tipbackLimit - gearAngleTol
    gearFailures = gearFailures + 1;
...
if isnan(rolloverActual) || isnan(rolloverLimit) || rolloverActual >= rolloverLimit - gearAngleTol
    gearFailures = gearFailures + 1;
...
if isnan(rotationSpeed) || isnan(takeoffSpeed)
    gearFailures = gearFailures + 1;
    logText = logf(logText, 'Unable to verify takeoff rotation capability due to missing gear data (check N20 and N21).\n');
elseif rotationSpeed >= takeoffSpeed - gearSpeedTol
    gearFailures = gearFailures + 1;
    logText = logf(logText, 'Takeoff rotation possible at %.1f kts, which is greater than your takeoff speed of %.1f kts. Increase pitch authority or reduce the weight carried by the nose wheel to reduce your rotation speed.\n', rotationSpeed, takeoffSpeed);
end
if ~isnan(takeoffSpeed) && takeoffSpeed >= 200 + gearSpeedTol
    logText = logf(logText, 'Takeoff speed too high: %.1f kts (recommend < 200 kts).\n', takeoffSpeed);
end
